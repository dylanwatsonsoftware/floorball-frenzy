...
export function lerpState(current: GameState, snapshot: GameState, t: number): void {
  ...
  current.ball.vx = snapshot.ball.vx;
  current.ball.vy = snapshot.ball.vy;
  current.ball.vz = snapshot.ball.vz;
  ...
  for (const role of ["host", "client"] as const) {
    ...
    current.players[role].vx = snapshot.players[role].vx;
    current.players[role].vy = snapshot.players[role].vy;
  }
