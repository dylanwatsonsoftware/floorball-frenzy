...
function encodeSnapshot(s: GameState): Uint8Array {
  ...
  const writePlayer = (p: Player, offset: number) => {
    ...
    v.setFloat32(offset + 40, p.heat, true);
    v.setFloat32(offset + 44, p.heatModeMs, true);
    let pFlags = 0;
...
function decodeSnapshot(v: DataView): GameState {
  ...
  const readPlayer = (offset: number, id: string): Player => {
    const pFlags = v.getUint8(offset + 48);
    return {
      id,
      ...
      heat: v.getFloat32(offset + 40, true),
      heatModeMs: v.getFloat32(offset + 44, true),
      input: {
